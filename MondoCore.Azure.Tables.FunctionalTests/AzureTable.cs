...
namespace MondoCore.Azure.Tables.FunctionalTests
{
    [TestClass]
    [TestCategory("Unit Tests")]
    public class AzureTableTests 
    {       
        private ITable<Automobile> _table = new AzureTable<Automobile>("Cars", TestConfiguration.Load().ConnectionString!, "Make");

        private List<string> _idCollection = new List<string>();

        public AzureTableTests()
        {
        }
        ...
        public async Task Reader_Get_notfound()
        {
            ...
            await Assert.ThrowsExceptionAsync<NotFoundException>( async ()=> await _table.Reader.Get(Guid.NewGuid().ToString()));
        }
    ...
    public class Automobile 
    {
        ...
        public string?  PartitionKey => this.Make;
    }
}
